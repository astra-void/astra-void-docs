...
That model works because the hard parts are not in the copied file. Behavior — focus, layering,
presence, controlled state, `asChild` — comes from [Lattice UI](/lattice-ui/), a real dependency.
Styling comes from [Vela](/vela-rbxts/), which lowers `className` to Roblox properties at compile
time, so a component holds no `Color3` and no `UDim2` of its own. A Facet component is a Lattice
primitive wearing Vela classes; what you own is the composition, short enough to read in one sitting.
