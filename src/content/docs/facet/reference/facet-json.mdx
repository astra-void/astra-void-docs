...
textual — `~/lib/utils` → `shared/lib/utils`; without it the CLI computes a relative path from the
importing file's destination, so `~/lib/utils` inside `src/shared/ui/button.tsx` becomes
`../lib/utils`.
...
line. See [Updating copied components](/facet/guides/updating-copied-components/#pinning-a-registry-revision)
