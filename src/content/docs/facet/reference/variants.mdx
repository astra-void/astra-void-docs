...
The one runtime package a Facet component imports from Facet, built by `rbxtsc`. Everything else a
component needs is a Lattice primitive or a Vela class string.
...
`className`. The caller lands last so an override wins by construction, which is why `cn` need not
merge conflicts and why nothing may be appended after `props.className`. See
[the one rule](/facet/guides/variants-and-classes/#the-one-rule).

Compound variants match the **resolved** selection, defaults included, so one fires whether the axis
...
Flattens `ClassValue`s into a space-separated string: strings pass through, numbers stringify,
arrays recurse, records contribute keys whose value is `true`, and everything else is dropped.

<Callout title="cn does not resolve conflicting utilities">
Unlike `tailwind-merge`, which of `p-2 p-4` wins is Vela's call, not this package's. Vela resolves left to right and a later token overwrites an earlier one, so class order *is* specificity. A merge pass would cost a token table tracking every Vela family, kept in sync with a compiler still adding families. It would reproduce an outcome that is already correct.
...
component's props. `props.className` inside a Facet component carries Vela's type no matter what the
component declared. A narrower mirror simply fails to accept it.
...
There is also a type-level trap in `fv`'s signature. The `Variants` default is
`Record<never, never>` rather than `VariantShape`. Mapping over `VariantShape`'s string keys
produces an index signature of `string | undefined`, which then collides with the `className` slot
on the selection object. Without that default, `fv("…")` with no variants fails to type-check at
every call site.