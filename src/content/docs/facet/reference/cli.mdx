...
`@rbxts/services` import that may not exist, so the CLI reuses the expression that is there and
stops when there is none.

<Callout title="Not being asked is not the same as saying no">
The prompt defaults to yes and is the last thing `add` does; `--yes` writes without asking. A **non-interactive** run without `--yes` writes nothing and prints the snippet — stdin not being a terminal is not consent, and not the same as declining.
</Callout>
