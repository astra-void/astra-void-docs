...
so with the token list — rather than letting the next build report it as a Vela diagnostic on a file
you never wrote.
...
today only [`dialog`](/facet/components/dialog/) — `add` offers to wrap your client entry in it.
This is the **one file the CLI edits that it did not write**, and the bar is deliberately high.
...
Why this and nothing else: everything else the CLI reports is a **build-time** failure, loud and in
front of the person who just ran the command. A missing `PortalProvider` compiles, type-checks,
passes CI, ships — and throws the first time a player opens the dialog.

**The parser is used for positions, never for output.** `@babel/parser` answers where the last
import ends and where the render call's argument starts; the edit itself is two string splices, so
everything outside those offsets comes out byte for byte identical. A test asserts it.
...
Writing `Players.LocalPlayer.WaitForChild("PlayerGui")` would also mean adding `Players` to an
`@rbxts/services` import that may not exist — a second, riskier edit for a situation that barely
occurs, since a client that mounts React already names its `PlayerGui`. So the CLI reuses the
expression that is there and stops when there is none.

<Callout title="Not being asked is not the same as saying no">
The prompt defaults to yes and is the last thing `add` does, so nothing scrolls past it; `--yes` writes without asking.

A **non-interactive** run without `--yes` writes nothing and prints the snippet, reported as *"nothing here to ask, and this is not a file to edit unasked"*. stdin not being a terminal is not consent. It is also not you having declined, which is why the two read differently.
</Callout>
...
`PortalProvider` compiles, ships, and throws the first time a player opens it, and nothing in the
copied file can prevent that. When it fails it prints the snippet; when it cannot find a single
client entry it warns rather than failing.
...
the shapes Facet writes. Anything it cannot read is reported as **unverifiable** rather than guessed
at.
