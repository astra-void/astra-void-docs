...
installed, whether their tokens resolve, and whether the packages underneath meet the floors those
files need. Most of what follows is something `doctor` would have told you.
...
`facet init` and `facet doctor` both check for this textually and neither edits the file — roblox-ts
tsconfigs are routinely JSONC, and a pattern-matched edit that mangles one is worse than a printed
snippet.
...
For a container, check the whole chain rather than the outermost level: one that hugs its content
can only measure children that already know their own size, so a single label with an unresolved
axis collapses everything above it.
...
computed expression that Vela resolves at runtime rather than at compile time. That runtime path was
a strict subset of the static lowering for a long time:
...
`vela-rbxts@^0.9.0` floor clears the whole list, and `facet doctor` verifies you are on it.
...
every transformed file and spent roughly 96 of Luau's 200 local registers before the file's own
first line. `card` does not fit in what is left.
...
On a component you wrote, the same limit applies: each exported name costs a register, which is why
`card` exports one `cardVariants` object rather than six recipes.
...
accepts a `className` and re-applies it inside drops them. That is why `TextSlot` takes no
`className`.
...
into from a strict context, so everything before this succeeds — it compiles, type-checks and ships
— and the failure lands on a player pressing the button.
...
One wrapper for the whole app, not one per dialog. `facet add dialog` offers to write it —
...
[dropped silently](/vela-rbxts/guides/colors-and-surfaces/#borders). What is left is
`border-border`, colouring a stroke that draws on all four sides.
