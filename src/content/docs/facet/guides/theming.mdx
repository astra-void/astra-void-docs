...
The previews on this site are built the same way, and show it. Flipping the docs' own light/dark toggle **reloads the frame against a different build** rather than repainting the one that is running: there is nothing in a running Facet tree that a theme signal could reach.
</Callout>
