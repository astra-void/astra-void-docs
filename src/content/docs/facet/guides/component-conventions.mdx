...
The conventions every component in the registry follows. They matter because you will edit the
copied files and write your own beside them.
...
`card` is the clearest case: every part carries `w-full h-fit`, all the way down.
...
one must *replace* those classes rather than add to them.

So do not add a wrapper frame just to get a second layout — restructure the parts, which you can do
freely because the file is yours. Generally: **do not add an instance to carry what an existing
instance can carry**. That is why the registry has no `AspectRatio` component:
...
and a level to the `AutomaticSize` chain to deliver what you can already write on the element you
have — and be less capable, since the class works on any host instance while a component forces a
`Frame`.
...
`Card`, `CardHeader`, `CardTitle` — not `Card.Root`. Namespace objects are right for a library; this
is source someone pastes and edits, so each part reads independently and can be deleted on its own.
...
`local` costs one — including every `export const`. On a Vela version that inlined its runtime into
each file, `card`'s six exported recipes pushed the module over 200 and it stopped loading, with an
error naming generated code its author never wrote:
...
CLI installs. The headroom is much larger — still finite, and still measurable:
...
the moderation and licensing forever. Facet renders `▾`, `✓`, `✕` as text and exposes the slot, so a
project wanting real artwork passes its own through `children`.
