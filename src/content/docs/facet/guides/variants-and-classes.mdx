...
Resolution order inside a recipe: **base → each matching variant → matching compound variants → the
caller's `className`**. The caller lands last, on purpose — which is the whole story of the next
section.

Recipes hold no colour, size or font value of their own — they name tokens, which Vela resolves from
your `vela.config.ts` at compile time. That is what makes a copied component themeable without
editing it.
...
Matched against the *resolved* selection, defaults included. A compound fires whether the axis came
from the caller or from `defaultVariants`.
...
stylesheet order and the later *rule* wins, so `p-4 p-2` and `p-2 p-4` are the same.
`tailwind-merge` exists to make attribute order mean what everyone assumes it does.

**In Vela**, tokens resolve left to right into instance properties and a later one overwrites an
earlier one, so class order *is* specificity. `fv()` appends the caller's `className` last, which
makes the behaviour `twMerge` simulates native.

"The caller" here is whatever code calls the recipe — the component itself. A `className` passed in
from *outside* is a different story. See
...
A merge pass would cost a token table tracking every Vela family, kept in sync with a compiler still
adding families. It would reproduce an outcome that is already correct.
...
If Vela ever stops resolving last-token-wins, this breaks silently: a consumer override that simply
does nothing. Worth re-reading whenever Vela's resolution order changes.

## Overriding from the call site

<Callout title="A className passed to a Facet component does nothing">
Measured, not predicted: a `<Button className="bg-destructive" Text="Override" />` rendered in the docs' own preview gallery comes out `bg-primary`, exactly like a `<Button />` with no `className`. The class is dropped in silence, with no diagnostic on either side.
</Callout>
...
`props.className` inside `Label` is `undefined`, so the recipe slot never sees it. The same
mechanism makes
[`TextSlot` take no `className`](/facet/guides/text-and-labels/#textslot-takes-no-classname) —
load-bearing there, in the way here.
...
**The answer is the model's answer: edit the file.** That is what a copied component is for, and the
only override that reliably works. `variant` and `size` cover the cases the component anticipated.
Anything else is a change to source you own.
