...
`facet init` writes `vela.config.ts` only when there is none, and merely *reports* on `tsconfig.json` rather than editing it. Both files belong to you, and a pattern-matched edit that mangles one is worse than a printed snippet.
...
defaults, and every upgrade inherits it. That is a large permanent commitment for one class in one
