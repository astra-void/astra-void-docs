---
title: Text field
description: Five parts around a TextBox, and a state that has to be stated twice.
product: facet
section: components
sidebar:
  order: 70
...
when pressed — the Roblox `<label for>`.
...
repetition [`Alert`](/facet/components/alert/#variant-goes-on-every-part) has with `variant`,
...
If the repetition bothers you in your project, the file is yours. Read the primitive's context in `TextFieldInput`, or collapse the five parts into one component that takes three strings.
</Callout>
