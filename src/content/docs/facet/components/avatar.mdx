...
`rounded-full` appears three times, on the root, on the image and on the fallback. It is not redundant. A `UICorner` rounds the instance it is on. It does not clip children to that shape.
