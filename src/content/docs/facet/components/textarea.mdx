---
title: Textarea
description: The one recipe that declares no height, because the primitive grows it line by line.
product: facet
section: components
sidebar:
  order: 80
...
there is resolved at that call site and arrives as instance properties. The component's own recipe
then overwrites *the properties it also names*. This recipe names no height, so
