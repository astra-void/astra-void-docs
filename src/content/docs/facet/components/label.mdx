...
lowered before `Label` ever sees it, then overwritten by the component's own recipe. See
...
Roblox's untouched default is LegacyArial. That is a different typeface, at a visibly different size
from the SourceSansPro everything else resolves to. Every text recipe in the registry declares a
weight for that reason. See
[Text and labels](/facet/guides/text-and-labels/#every-text-recipe-declares-a-font-).
