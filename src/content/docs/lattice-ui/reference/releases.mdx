...
- **UI modifiers are recognised under both renderers.** `Slot` looks each child's type up in a table of UI modifiers, and that table held one spelling at a time. `@rbxts/react` rewrites a host tag to its Roblox class name, while a browser React renderer leaves it as written. Both are now listed.
- **`UIShadow` counts as a modifier**, so a `shadow-*` class no longer fails `asChild` like a casing miss.
- **Re-parented modifiers no longer collide keys with the element's own children.** Both child walks numbered from `.0`, and React may resolve a duplicate key by dropping a child rather than warning.
...
- Overlay triggers and items are gamepad-selectable. The `Select` trigger and item, the toggle-group item and the accordion trigger register focus nodes and route activation through a shared guard, so gamepad users can open menus, move between items and commit without double-firing.
