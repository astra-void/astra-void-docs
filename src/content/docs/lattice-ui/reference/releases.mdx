...
All `@lattice-ui/*` packages version in lockstep, so a single release number describes the whole
library. This page summarizes each release and its migration steps. The exact code changes live in
[Migration](/lattice-ui/reference/migration/), and the full entry-by-entry history in the
repository's [CHANGELOG.md](https://github.com/astra-void/lattice-ui/blob/main/CHANGELOG.md).
...
- **UI modifiers are recognised under both renderers.** `Slot` looks each child's type up in a table of UI modifiers. That table held one spelling at a time. `@rbxts/react` rewrites a host tag to its Roblox class name, while a browser React renderer leaves it as written. Both are now listed.
- **`UIShadow` counts as a modifier**, so a `shadow-*` class no longer fails `asChild` like a casing miss.
- **Re-parented modifiers no longer collide keys with the element's own children.** Both child walks numbered from `.0`. React may resolve a duplicate key by dropping a child rather than warning.

<Callout title="If you are on 0.8.0 in Roblox, this changes nothing you can see">
  The 0.8.0 keying is correct for `@rbxts/react`, so a shipped game was never affected by the first fix. The other two are real anywhere.
</Callout>
...
A `CanvasGroup` composites its whole subtree into one offscreen layer so `GroupTransparency` can fade it as a unit. That is right for the one effect and wrong for everything else. Children could not carry their own transparency, and every dialog, menu and tooltip paid for the layer whether or not it faded. Rendering a `Frame` makes the composite opt-in.
</Callout>

**Highlights**

- `Dialog.Content` accepts `asChild` — the last content part without it. Pass a `canvasgroup` and it becomes the motion host, so `createCanvasGroupRevealRecipe()` fades the surface as one layer again. The outside-press boundary moves down to that element's first host child. See [Dialog](/lattice-ui/components/dialog/).
...
- `asChild` works in Roblox on a subtree a Tailwind-style transform has styled. `UI_MODIFIER_TAGS` was keyed by JSX tag, so every modifier missed the lookup and counted as a second target candidate. Any recipe emitting a `UIListLayout` or `UICorner` hit it on every use. 0.8.1 finishes the job by listing both spellings. See [`asChild` composition](/lattice-ui/guides/as-child-composition/).

## 0.7.0

The largest breaking release so far: every primitive stripped down to behavior, and the CLI rebuilt
around the same idea.

**Migration** — see [Migration](/lattice-ui/reference/migration/) for the full walkthrough. In short:

- Every primitive renders unstyled. Pass the appearance you want directly to the part, since props now forward onto the instance it renders, or use `asChild`.
...
<Callout title="This release mostly does not fail to compile">
  It renders differently — surfaces that looked finished on 0.6 come up flat, because the appearance came from the primitive. Plan a visual pass, not just a typecheck.
</Callout>

**Highlights**

- Primitives now set behavior plus the minimum needed to neutralize Roblox's own instance defaults. Geometry computed from state stays owned by the primitive: progress fill ratios, slider thumb travel, popper position, scroll thumb size.
...
A CLI-only patch. It changes no primitive behaviour. Every fix is about making a scaffolded project
install and type-check cleanly on npm, where a peer conflict is a hard error rather than a warning.

**Migration**

- Use `npx lattice-ui` rather than `npx lattice`. `npx` resolves the published package name, not the shorthand alias, so `npx lattice` never worked on npm projects. The CLI's own command hints now say `lattice-ui`, and the `lattice` binary still works once the package is installed.
- If your project still lists pre-0.6.1 package names, run `lattice init`. It rewrites them to their `react-` replacements instead of leaving both names to fight over peers. `doctor` reports them, and `upgrade` skips them.

**Highlights**

- `create` and `init` record the resolved package manager as a `devEngines` pin. `add`, `remove` and `upgrade` apply that pin before touching dependencies, so a project scaffolded with one manager cannot be quietly installed with another.
- `typescript` is pinned to the version roblox-ts actually compiles with, rather than resolved from the latest dist-tag. `doctor` warns when a project is already on an unsupported major.
...
A **rename-only** release. Every package moved under a framework layer directory and gained a layer
prefix in its published name, so future Vide and plain-Luau targets can sit beside React. No runtime
behaviour, exports or APIs changed.
...
- Overlay triggers and items are gamepad-selectable. The `Select` trigger and item, the toggle-group item and the accordion trigger register focus nodes and route activation through a shared guard. Gamepad users can open menus, move between items and commit without double-firing.
...
- Toast now runs on presence motion. It fades in on entry and stays visible while its exit animation completes. The expiry timer starts when it enters the visible window rather than at enqueue, and `clear()` animates through the exit transition.
...
- Dismissable layers re-promote on open for correct z-order and dismissal order, and dismiss on sunk input. They register their trigger as an inside ref, so clicking it no longer dismisses. Outside-click dismissal also works without an explicit content boundary ref.
...
- An interactive `init` CLI command for existing projects, with safe template merging. `create` and `init` scaffold from maintained package templates.
