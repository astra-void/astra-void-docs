...
A `CanvasGroup` composites its whole subtree into one offscreen layer so `GroupTransparency` can fade it as a unit — right for that one effect, wrong for everything else. Children could not carry their own transparency, and every dialog, menu and tooltip paid for the layer whether or not it faded. Rendering a `Frame` makes the composite opt-in.
</Callout>
