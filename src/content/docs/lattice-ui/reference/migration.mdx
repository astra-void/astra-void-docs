...
That means you need to know what each part renders. `Dialog.Overlay` is a `textbutton`, `Dialog.Content` is a `canvasgroup`, `Toast.Viewport` and `Menu.Group` are `frame`s. Passing `TextColor3` to a `frame` will not compile.
...
| --- | --- |
| `Dialog.Content` | `createCanvasGroupRevealRecipe()` |
| `Menu.Content`, `ContextMenu.Content`, `Combobox.Content`, `Tooltip.Content` | `createCanvasGroupPopperEntranceRecipe(placement)` |
| `Popover.Content`, `Select.Content` | `createCanvasGroupPopperEntranceRecipe(placement)`, or `createPopperEntranceRecipe(placement)` under `asChild` |
...
transition={{
  ...
  reveal: { values: { GroupTransparency: 0 }, intent: { duration: 0.12 } },
  exit: { values: { GroupTransparency: 1 }, intent: { duration: 0.096 } },
}}
```
...
- **`asChild` accepts UI modifiers as siblings.** A `UICorner`, `UIPadding` or `UIListLayout` written next to the child — the shape a Tailwind-style `className` transform emits — is now re-parented under the element the props land on, instead of raising "asChild requires a child element". Fragments are looked through; two real candidates are still an error.
