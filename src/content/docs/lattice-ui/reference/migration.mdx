...
`Dialog.Content` is the one part whose old default cannot be passed back. Its motion host is a `frame` rather than a `canvasgroup`, and a `frame` has no property that fades its descendants, so `createCanvasGroupRevealRecipe()` has nothing to write `GroupTransparency` to. Give the content a `Position`-only config for the rise and fade the overlay dim or your own panel elements instead — [Fading a dialog](/lattice-ui/components/dialog/#fading-a-dialog) has the pattern.
