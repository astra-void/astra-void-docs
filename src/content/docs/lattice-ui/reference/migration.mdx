...
If you ship to Roblox, 0.8.0 already worked. Under browser React — which leaves a host tag as
written where `@rbxts/react` rewrites it to a class name — `asChild` failed on every styled subtree
until this release.
...
Tailwind-style transform emits counted as a second target candidate and any subtree with a
`UICorner` sibling failed. Delete whatever you did to avoid it. Under a browser React renderer, see
[0.8.0 → 0.8.1](#080--081).
...
sizes, font sizes, literal text and decorative children are gone — and any prop the underlying
instance accepts forwards straight through, so styling no longer requires `asChild`.
...
Geometry the primitive *computes from state* stays with the primitive: progress fill ratios, slider thumb travel, popper-driven position, scroll thumb size, and the full-screen sizing that makes an overlay cover the screen.
...
No **presence** motion runs unless you ask for it. Timing is unchanged — content stays mounted until
an exit transition finishes — but with no `transition` it is instant. Motion that follows a value
...
The popper recipes took the **resolved** placement, so motion originated from the side the surface landed on after any flip. Read it from `usePopper` to get that back; passing no placement animates from a fixed side.
