...
If you ship to Roblox, 0.8.0 already worked. If you render Lattice primitives under browser React — which leaves a host tag as written where `@rbxts/react` rewrites it to a class name — `asChild` failed on every styled subtree until this release.
...
`GroupTransparency` and `GroupColor3` are compile errors there. Both were the only reason to know
the host was a `CanvasGroup`.
...
full-screen rectangle. Two ways out — `Dialog.Content` accepts `asChild` as of this release to make
the second possible:
...
Tailwind-style transform emits missed the lookup and counted as a second target candidate — any
subtree with a `UICorner` or `UIListLayout` sibling failed with "expected exactly one child element
besides any UI modifiers". Delete whatever you did to avoid it. Under a browser React renderer, see
[0.8.0 → 0.8.1](#080--081).
...
The largest breaking release so far. Every primitive is stripped down to behavior — colors, fixed
sizes, font sizes, literal text and decorative children are gone — and in exchange any prop the
underlying instance accepts forwards straight through, so styling no longer requires `asChild`.

<Callout title="Expect a visual diff, not a broken build">
Most of this release does not fail to compile — it renders differently. A dialog that looked finished on 0.6 comes up as an invisible overlay and an unstyled panel. Budget a pass over every surface, not just the ones the compiler flags.
</Callout>

### 1. Every primitive renders unstyled

Primitives set behavior plus the minimum needed to neutralize Roblox's instance defaults — a bare `textbutton` renders an opaque grey box labelled "Button", so parts still set `BackgroundTransparency = 1`, `Text = ""` and friends. Everything past that is yours.
