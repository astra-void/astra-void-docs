...
`Popover.Content` runs no motion of its own. Pass a `transition` to animate it, and `forceMount` when the node should stay mounted instead of unmounting after the exit finishes — useful when you drive motion yourself or need the instance to persist. The non-`asChild` wrapper is a `canvasgroup`, so `createCanvasGroupPopperEntranceRecipe` is the recipe that keeps the `GroupTransparency` fade.
...
`Popover.Content` runs no motion unless you pass a `transition`. The placement-aware recipes in `@lattice-ui/react-motion` are what it used to apply: `createCanvasGroupPopperEntranceRecipe(placement)` slides the surface 10 pixels in from the resolved placement side while fading `GroupTransparency`, which matches the `canvasgroup` wrapper; `createPopperEntranceRecipe(placement)` does the same through `BackgroundTransparency` for the `asChild` path, where your element replaces that wrapper. Both take their own distance and duration, and exit at 0.8x the reveal.
...
<PropTable
  rows={[
    ...
    { name: "asChild", type: "boolean", description: "Position and animate the single child element instead of the auto-sized canvasgroup wrapper the part renders. Pair it with createPopperEntranceRecipe rather than the canvas-group variant." },
    ...
    { name: "transition", type: "PresenceMotionConfig", description: "Reveal/exit motion. None by default; createCanvasGroupPopperEntranceRecipe(placement) matches the canvasgroup the content renders, createPopperEntranceRecipe(placement) suits the asChild path." },
