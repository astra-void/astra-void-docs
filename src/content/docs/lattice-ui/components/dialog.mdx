...
The host is a plain `Frame` spanning the whole layer, and a `Frame` has no property that fades its descendants — `BackgroundTransparency` on the host fades the host's own background, which covers the screen. Animating it through `transition` fills the screen with a rectangle instead of fading your panel, so keep the content transition to `Position` and put the fade where the pixels are:
...
<Callout title="Recipes that animate GroupTransparency do not fit Dialog.Content">
`createCanvasGroupRevealRecipe` and `createCanvasGroupPopperEntranceRecipe` animate `GroupTransparency`, which exists only on a `CanvasGroup`. No primitive renders one any more: the anchored parts take a `canvasgroup` you pass through `asChild`, and `Dialog.Content` has no `asChild` at all, so on it the write has no property to land on.
</Callout>
...
<PropTable
  rows={[
    ...
    { name: "…Frame props", type: "Partial<WritableInstanceProperties<Frame>>", description: "Forwarded onto the rendered frame and type-checked against it. The frame spans the layer, so styling it paints the full screen — style your panel child instead. A transition owns Position while it runs." },
  ]}
/>
