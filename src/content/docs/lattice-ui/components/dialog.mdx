...
`Dialog.Content` renders your children inside a full-screen `CanvasGroup`, but runs **no motion of its own**. Pass a `transition` to animate it; the config you pass is used as-is, with nothing underneath to merge with. `createCanvasGroupRevealRecipe()` from `@lattice-ui/react-motion` is the natural fit, since it animates `GroupTransparency` and so fades the panel as one surface:
