...
drags and keyboard input into value changes, so your component only renders the track, the filled
range and the thumb.
...
the value. For a "save on release" control use `onValueCommit`, which fires once when the
interaction ends.
...
`min`, `max` and `step` shape the whole value space: every value is clamped to `[min, max]` and
snapped to the nearest multiple of `step` counted from `min`. A slider from 0.1 to 2 in 0.05
increments lands only on 0.10, 0.15, 0.20 and so on. Format the readout yourself — the value is a
plain number.
...
props; the thumb slot also centres your element with a forced `AnchorPoint` of (0.5, 0.5).
`Slider.Range` is different — your child is stretched to fill the animated fill frame, so style it
with colour, gradients and corners rather than sizing it.
...
by `step * 10`, and `Home`/`End` jump to the bounds. Each of those both changes **and commits**, so
a keypress-heavy control fires `onValueCommit` once per press — keep that handler cheap or debounce
it. `Return`/`Space` commits without changing the value.
...
`defaultValue` (itself defaulting to `min`). Every value is clamped and snapped as above.
`min`/`max` default to `0`/`100` and are normalized so the lower bound is the smaller of the two;
`step` defaults to `1`, and a zero or negative `step` falls back to `1`.

### Change vs. commit

`onValueChange` fires as the value moves — every drag tick and value-changing keypress.
`onValueCommit` fires at the end of an interaction: on drag release, after each keyboard adjustment,
or on `Return`/`Space`. Treat change as the live-preview channel and commit as the persist channel.

### Rendering and layout

`Slider.Root` renders no instance, so `Slider.Track` is the outermost GuiObject and sits directly in
the parent layout. The default track is a `frame` sized 260x10 or 10x220, the range an animated fill
`frame`, the thumb a 16x16 `textbutton` anchored at its centre — all three replaceable with
`asChild`.

### Dragging

A press on either `Track` or `Thumb` starts a drag and jumps the value to the pressed position.
While dragging, the root follows `UserInputService` input changes — even after the pointer leaves
the track — then commits on release. Touch drags are tracked per input object, so multi-touch does
not cross wires.
