...
`Menu.Content` runs no motion of its own. Pass a `transition` to animate it — `createCanvasGroupPopperEntranceRecipe(placement)` matches the `canvasgroup` the content renders, and building it from the **resolved** placement makes a menu the popper flipped above the trigger animate from above. `forceMount` keeps the content mounted through its exit (useful when you drive motion yourself or need the node to persist). The content wrapper is an automatically-sized `canvasgroup`, so your surface defines the measured size.
