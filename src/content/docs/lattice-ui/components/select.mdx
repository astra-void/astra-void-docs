...
`Select.Content` runs no motion of its own. Pass a `transition` to animate it: `createCanvasGroupPopperEntranceRecipe(placement)` matches the `canvasgroup` the content renders, and `createPopperEntranceRecipe(placement)` suits the `asChild` path where your own element replaces it. Both travel from the resolved placement side. `forceMount` keeps the content mounted while closed and through its exit.
...
<PropTable
  rows={[
    ...
    { name: "transition", type: "PresenceMotionConfig", description: "Reveal/exit motion. None by default; createCanvasGroupPopperEntranceRecipe(placement) matches the canvasgroup the content renders, or createPopperEntranceRecipe(placement) under asChild." },
    ...
    { name: "asChild", type: "boolean", description: "Render onto the single child element instead of the canvasgroup the part renders. Pair it with createPopperEntranceRecipe rather than the canvas-group variant." },
