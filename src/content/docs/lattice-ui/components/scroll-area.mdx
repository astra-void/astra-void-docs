...
Scroll Area builds a scroll container with custom-styled scrollbars on top of Roblox's native
`ScrollingFrame`. `Viewport` is the scrolling surface, `Root` reads its canvas metrics and decides
when scrollbars show, and `Scrollbar`/`Thumb` render a draggable indicator wired to
`CanvasPosition`. Reach for it when you want the **native scroll feel** — wheel, touch drag,
momentum — with **scrollbars you style yourself** and **overflow-aware visibility**.
...
`Scrollbar` per axis, with a `Thumb` directly inside its track, and a `Corner` when you show both.
...
The default viewport, scrollbar track and thumb. Because `Root` renders no instance, the parts sit
in a plain container frame you provide. Wheel and touch scrolling work immediately, and the
scrollbar appears only once the list overflows.
...
A single row that overflows sideways — hotbars, category chips, carousels. Pass
`orientation="horizontal"` to both `Scrollbar` and `Thumb`, and use `asChild` to size the viewport
and track into your layout.
...
Content that overflows in both directions — maps, skill trees, wide tables. Provide one
scrollbar/thumb pair per axis plus a `Corner`, which shows only while both scrollbars are visible.
Each track is shortened by the other's thickness so they meet at the corner instead of crossing.
...
overflows. `"scroll"` is transient: the bar appears on scroll activity and fades after
`scrollHideDelayMs` — a good fit for chat logs and feeds. In every mode, an axis with no overflow
shows nothing.
...
on the thumb — the primitive owns them — and keep it a direct child of the track, which it measures
to map drags to canvas positions.
...
`ScrollingDirection` set to XY, and the native scrollbars hidden. It registers the frame with `Root`
and pushes the per-axis `viewportSize`, `contentSize` and `scrollPosition` into context on every
change, with an equality bail-out so identical measurements never re-render. Because the control is
a real `ScrollingFrame`, wheel, touch-drag and momentum scrolling all work natively. Use `asChild`
to supply your own `scrollingframe` sized to your layout.
