...
Override the animation with `transition`; the default recipe always settles at 12x12, so an indicator of any other size should pass `createIndicatorRevealRecipe` (from `@lattice-ui/react-motion`) built with its own size. Pass `forceMount` to keep the indicator mounted while unchecked and through its exit, bypassing the presence wrapper so you can drive visibility yourself. With `asChild`, your child element is rendered in place of the default 12x12 frame and its `Visible` property is bound to the presence state.
