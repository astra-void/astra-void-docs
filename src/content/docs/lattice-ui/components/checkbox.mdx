...
checked state, the toggle logic and its indicator's reveal and exit motion. Reach for it for
settings toggles, opt-ins, multi-select lists and "select all" headers. For a plain boolean flip
with a sliding handle, [Switch](/lattice-ui/components/switch/) fits better.
...
label of its own, so nothing is drawn until you give it a size and colours.
...
in, pass a `transition` built with `createIndicatorRevealRecipe` at its actual size.
...
indeterminate state yourself through controlled `checked`, typically computed from child selections
— the toggle never produces it.
