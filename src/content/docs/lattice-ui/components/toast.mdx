...
renders each surface and reacts to actions. Reach for it when messages should **stack and expire on
their own**, stay **capped to a few at a time**, and **animate out** without you tracking timers.
You drive it imperatively with `useToast` from anywhere inside the provider.
...
`Toast.Viewport` renders its children and nothing else, so a bare viewport shows nothing. Map `useToast().visibleToasts` onto `Toast.Root` yourself; `Root`, `Title`, `Description`, `Action` and `Close` are how you build each surface.
</Callout>
...
`onExitComplete` to `finalize` so a toast is dropped only once its exit finishes.
...
There is no `open`/`onOpenChange` on a toast — you `enqueue` and `remove`, and the provider decides what is visible and when each leaves. Treat the queue as the source of truth, not local component state.
</Callout>

### Undo action toast

`ToastOptions` carries only data — `id`, `title`, `description`, `durationMs` — never callbacks. Key
the pending undo by the id `enqueue` returns, then wire `Toast.Action` to the stored payload while
mapping `visibleToasts`.
...
own `id` so a different code path can clear it later, like a "connection lost" warning on reconnect.
...
One caveat: a toast's clock starts at `enqueue`, but expiry is only *evaluated* while it is inside
the visible window, so one that waited longer than its `durationMs` exits almost immediately on
surfacing. Under bursts, lengthen the duration, collapse the burst into one summary toast, or go
sticky with manual removal.
...
The viewport takes no position props of its own, so put it inside an anchored wrapper frame to
choose its corner. The stack direction and gap come from the layout you put inside it. Reach for
`Viewport asChild` when you need a different instance class than the `frame` it renders.
...
`Toast.Root` runs no motion unless you pass a `transition`. `createToastResponseRecipe()` animates `BackgroundTransparency` between `0` and `1` over 0.14s; rebuild it with a new duration or write a `ResponseMotionConfig` by hand.

Keep the duration at or under about 0.16s — the provider drops exiting toasts after a fixed ~160ms window regardless of your transition, so a slower fade is cut off mid-motion.
...
`enqueue(options)` appends a toast and returns its `id`, auto-generated as `toast-N` unless you pass one. `remove(id)` starts its exit, or drops it instantly if it is still waiting beyond the visible cap, and is a no-op on one already exiting. `clear()` empties the queue immediately, skipping exit motion.

While any toasts exist the provider runs a `Heartbeat` connection that prunes expired and finished toasts each frame, disconnecting once the queue drains — so an idle provider costs nothing.
