...
Override the animation with `transition` (any `PresenceMotionConfig`, such as one built with `createSurfaceRevealRecipe` from `@lattice-ui/react-motion`), or pass `forceMount` to keep the body mounted while closed and through its exit, bypassing the presence wrapper so you can drive motion or measurement yourself. With `asChild`, your single child element is rendered in place of the default frame and its `Visible` property is bound to the presence state.
