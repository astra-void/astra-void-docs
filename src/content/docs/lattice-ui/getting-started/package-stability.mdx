...
The current release is `v{LATTICE_VERSION}`. The `v{LATTICE_MINOR}.x` line drew the line between behavior and appearance: every primitive now ships unstyled, props forward onto the instance a part renders and are type-checked against it, and motion only runs when you pass a `transition`. That settles the question each primitive's API was previously answering inconsistently — how much of the look belongs to the library — and it is what the stable-direction surface will carry into `v1.0`. See [Migration](/lattice-ui/reference/migration/) if you are coming from `0.6`.

The `v0.6.x` line before it focused on hardening: more reliable layered and composite primitives, steadier motion and exit transitions, a real keyboard-navigation foundation rather than leaning on Roblox default selection, stronger focus restoration and trapping, and wider regression coverage.
