...
On Roblox, "focus" is gamepad and keyboard **selection**: a single `GuiObject` held by `GuiService.SelectedObject`. There is no focus tree and no tab order. The moment you layer surfaces, that one property becomes contended — open a modal and selection should move into it, close it and selection should land back on whatever opened it, and meanwhile the gamepad should not wander out.
...
- A **focus scope** is a region — a dialog body, a menu list — with rules about activity, trapping and restoration. `FocusScope` registers one and shares its id through context, so any `useFocusNode` inside belongs to it.

The manager resolves which node is focusable and writes it to `GuiService.SelectedObject`; when the player moves the gamepad it reads the change back and re-resolves. That two-way bridge is the point — your model and Roblox's selection stay in agreement.
