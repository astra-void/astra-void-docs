...
Every row above is `Switch.Root` + `Switch.Thumb` with no styling of its own. Open the **Classes**
tab for the source and **Lowered** for the props Vela produced.
...
an ordinary `textbutton` — so its `className` is lowered exactly as it would be anywhere else.

```tsx title="Classes on the element you own"
<Switch.Root asChild checked={wifi} onCheckedChange={setWifi}>
  <textbutton className="w-11 h-6 rounded-full bg-sky-500" Text="">
...
**Dynamic class values.** A computed `className` moves the element onto Vela's
[runtime path](/vela-rbxts/guides/dynamic-class-names/), where the in-game resolver understands a
fraction of the utility set and drops the rest silently — layout direction, alignment, text styling
and position among them. It also stops merging the sizing axes: `w-11 h-6` in a computed string
leaves one axis at zero. Since primitive state (`checked`, `highlighted`, `disabled`) is exactly the
thing you want to paint from, this matters here more than usual. Branch between two fully static
