...
0.7.0 added, and **0.8.0 is the release where it works**. `Slot` re-parents the modifier under the
element the props land on instead of treating it as a second slot candidate, so this renders the same
tree as the first form. On 0.7.x the rule shipped keyed by the JSX tag rather than the Roblox class
roblox-ts actually emits, so every modifier missed it and the whole shape raised "expected exactly one
child element besides any UI modifiers". See
...
[runtime path](/vela-rbxts/guides/dynamic-class-names/) that a computed value puts the element on, so
