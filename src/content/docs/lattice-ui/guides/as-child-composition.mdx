...
**On 0.7.x that second rule is documented but inert.** The lookup that recognises a modifier was
keyed by the JSX tag, and `@rbxts/react` rewrites a host tag to its Roblox class name before it
builds the element — `<uicorner />` reaches `Slot` as `"UICorner"`. Every modifier therefore missed
the lookup and counted as a second candidate, so `asChild` failed with "expected exactly one child
element besides any UI modifiers" on precisely the subtrees the rule was written for. Fixed in
0.8.0.
...
That second rule exists because Roblox attaches modifiers as *children* rather than properties, which is exactly the shape a Tailwind-style `className` transform emits when it lowers `rounded-md` or `p-2` at the call site — [vela-rbxts](/vela-rbxts/) being the one this was built against:
...
This is what makes a class name on the *part* work: Vela emits its helper instances as the
component's children, and they arrive here as siblings of your element rather than as rival slot
candidates. [Styling with Vela](/lattice-ui/guides/styling-with-vela/) walks through all three
placements.
</Callout>
