...
**On 0.7.x that second rule is documented but inert.** The lookup that recognises a modifier was keyed by the JSX tag, and roblox-ts labels a host element with its Roblox class — `<uicorner />` reaches `Slot` as `"UICorner"` — so every modifier missed it, counted as a second candidate, and `asChild` failed with "expected exactly one child element besides any UI modifiers" on precisely the subtrees the rule was written for. Fixed in 0.8.0.
