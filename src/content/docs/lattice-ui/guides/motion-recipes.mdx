...
`Dialog.Content` is the one part with no recipe to reach for. It renders a full-screen `Frame`, so a transition can slide it but has no property that fades its descendants — write a `Position`-only config for it and fade the overlay or your own panel elements separately. See [Fading a dialog](/lattice-ui/components/dialog/#fading-a-dialog).
