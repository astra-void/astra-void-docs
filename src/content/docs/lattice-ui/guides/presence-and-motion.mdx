...
`Dialog.Content` is the exception: its host is a full-screen `Frame`, so give it a `Position`-only config and fade the overlay or your own panel elements instead — see [Fading a dialog](/lattice-ui/components/dialog/#fading-a-dialog).
