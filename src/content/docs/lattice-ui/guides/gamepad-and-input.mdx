...
session, and lattice-ui primitives are built so one composition serves all of them. Pointer and
touch activate controls directly, keyboard arrows move through items in a deterministic order, and
gamepad navigation rides Roblox's selection engine. This page covers how movement and activation
reach your components; for scopes, trapping and restore, see
...
- **The focus bridge reads movement back.** While any `FocusScope` is active the manager listens to `GuiService.SelectedObject` changes and updates the model to match — pulling selection back inside if a trapped scope is active and it escaped. See [Focus management](/lattice-ui/guides/focus-management/).
- **`SelectionGained` drives your visuals and state.** Tabs and RadioGroup select their value the moment selection lands on an item, and Menu highlights it with the same handler it uses for `MouseEnter`.
