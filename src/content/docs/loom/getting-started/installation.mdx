...
Loom ships as two packages, and **you rarely need both**:
...
self-contained. Pick the row that matches how you want to run the preview:
...
Only the plugin has peers; the CLI vendors its own copies. `@loom-dev/preview` declares two:
...
<Callout title="The React peer range is wrong — install React 18">
`|| ^19.0.0` in that range is a packaging bug, not an option. The React adapter drives
`react-reconciler@^0.29.2`, which reads React 18 internals that React 19 renamed, so a React 19
install fails at evaluation time. Until the range is narrowed, treat it as `^18.3.1`:
