...
Install only `@loom-dev/preview` if you keep a `vite.config.ts` — `plugins: [loomPreview()]` is the
entire integration, and `vite` / `vite build` do the rest. Install `loom-dev` too if you would rather
have no Vite project at all.
