...
With `--targets` present, loom stops looking for a client entry entirely — the gallery shell does the
...
The sidebar header carries a **`debug`** button — also **Ctrl+Alt+D**, also `?debug=1` — that opens
a panel over the stage reporting what the mounted target is actually doing: import and first-frame
