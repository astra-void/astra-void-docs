...
**Your own aliases resolve at runtime too, since `0.12.0`.** The preview plugin reads `baseUrl` and
`paths` out of the project's `tsconfig.json`, so a project whose source imports non-relatively —
`import { Button } from "@/ui/Button"` — starts at all. Before that release those imports resolved
during a typecheck and nowhere else, and the preview failed on the first one it hit.
