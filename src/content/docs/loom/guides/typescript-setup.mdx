...
`tsconfig.json` for anything but a couple of transform flags, so TypeScript is purely an editor and
CI concern — you are making a *second* typecheck configuration for previewing, not modifying the one
that produces Luau.
...
rest — and the Luau environment: `task`, `math`, `string`, `os`, `coroutine`, `typeIs`, `pcall`,
`pairs`, `tostring`, `error`, `warn`.
...
does not already exist — which is why the globals import lives in a separate file.
...
(`React.Event`, `React.Change`) are not in its types and a keyed-prop namespace types as an error
even though it works — the `Event={{ Activated: fn }}` prop form avoids that.
...
the project's `tsconfig.json`, so a project whose source imports non-relatively —
