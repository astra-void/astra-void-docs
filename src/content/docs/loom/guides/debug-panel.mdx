...
**viewport** — four numbers that agree on a desktop and diverge on a phone, which is the reason all
four are shown: the stage in real pixels, the **logical viewport the scene actually laid out
against**, `Workspace.CurrentCamera.ViewportSize`, and the mobile scale factor with the `?base=` width
behind it. Plus the device pixel ratio and the theme. [Mobile previews](/loom/guides/mobile-previews/)
explains why those diverge.
