...
Before 0.3.0 the globals were dev-server-only. A stock `vite build` compiled cleanly, shipped, then threw `ReferenceError: UDim2 is not defined` on first render. The fix was writing `import "@loom-dev/preview/globals";` as the first import of your entry. That import is now
