...
| `rbxassetid://` images are blank in a static build | The id is assembled at runtime, so the build could not find it to bake | Pass a real URL, or install your own `setImageResolver` — see [static builds](/loom/guides/static-builds-and-embedding/#rbxassetid-in-a-static-build) |
