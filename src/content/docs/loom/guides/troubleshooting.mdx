...
| `no face is loaded for the Roblox font family "…"` | The browser has none of the engine's typefaces | `import "@loom-dev/renderer/fonts"`, or [register your own](/loom/guides/fonts/) |
| Text wraps differently on another machine | The same thing — a different fallback font per OS | As above |
...
`UIGradient.Offset`, `ScrollBarThickness`. Images paint as of `0.6.0`, but `ScaleType.Slice` /
`ScaleType.Tile` fall back to `Stretch` and the `ImageRectOffset` sprite window is not implemented at
all, so an icon cut out of a sheet renders as the whole sheet. `UIPageLayout` / `UITableLayout` are
recognized but do nothing.
...
A browser ships none of Roblox's typefaces. Unregistered, every Roblox family resolves to the
machine's `system-ui` — SF Pro, Segoe UI or Roboto depending on the OS — and since `AutomaticSize`
and `TextWrapped` are driven by measuring glyph widths, the *layout* moves with the font, not just
the paint.

```ts title="The one-line fix for the openly licensed families"
import "@loom-dev/renderer/fonts";
```

`Gotham` is proprietary and cannot ship with loom; register your own copy with `registerFont`. Both
are [Fonts and text metrics](/loom/guides/fonts/).
