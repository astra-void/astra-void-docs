---
title: Luau globals
description: The Luau standard library a previewed tree runs against, and what is missing on purpose.
product: loom
section: reference
sidebar:
  order: 28
...
the number written in the source is already a Luau index. Loom reads it as one.
...
`gsub` takes a **function or a table** as its replacement, not only a string. `format` understands
the Lua specifiers a UI reaches for: width, padding and precision.
...
roblox-ts also calls these off a string receiver. `String.prototype` carries `.lower()`, `.upper()`,
`.sub()`, `.rep()`, `.find()`, `.gsub()`, `.format()`, `.gmatch()`, `.byte()`, `.len()`,
`.reverse()` and `.size()`. Each delegates to the library above, with the same 1-based indices and
tuple returns. `.sub()` deliberately replaces the Annex B HTML wrapper JavaScript ships under that
name.
...
`string.match` would change behaviour far outside the previewed tree. Call `string.match(s, …)` instead. `.split()` is left native for the same family of reasons, since Luau's `string.split` is implemented *with* it.
</Callout>
...
resolve on `Map` and `Set` through a symbol the preview rewrites previewed source to. Every other
...
to a deterministic generator. Code that seeds for reproducibility gets it, rather than being
silently ignored. A scene wanting the same "random" layout every reload works.

`math.log` takes an optional base, and `frexp` / `modf` return their pair as a destructurable array.

## `os.date`

A strftime subset: `%a %A %b %B %c %d %H %I %j %m %M %p %S %x %X %y %Y %%`. An unrecognized
specifier is left as written. The `*t` and `!*t` table forms and the `!` UTC prefix work on any
format. `os.time` accepts a date table as well as no argument, and `os.difftime` subtracts two of
its results.

## `debug` profiling shows up in devtools

`debug.profilebegin` and `profileend` are wired to `performance.mark` and `performance.measure`.
Roblox instrumentation a component already carries appears as real entries in the browser's
Performance panel. `debug.traceback` returns the JavaScript stack, which is the real one here.
...
`buffer` is little-endian and bounds-checked, so an out-of-range access throws as it does in the
engine. `typeOf` answers `"buffer"` for one, since Luau's buffer is a distinct primitive type rather
than an `ArrayBuffer` with a different name.
...
behaviour without proxying every table in the program. A half-faithful one would be worse than the
honest gap: a scene would render right up until the moment the metatable mattered.
...
**No filtering, no networking.** Anything that would need a Roblox server throws by name rather than
quietly doing nothing. That covers `HttpService.GetAsync` and `TextService.FilterStringAsync`. See
