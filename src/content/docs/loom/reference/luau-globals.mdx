...
`.reverse()` and `.size()`, each delegating to the library above with the same 1-based indices and
...
specifier left as written. The `*t` and `!*t` table forms and the `!` UTC prefix work on any format.
