...
Before that release `luau.ts` left out `table`, which UI code reaches for most: a component building its rows with `table.insert` died on `table is not defined`. `bit32`, `utf8`, `debug` and `buffer` arrived in the same release.
</Callout>
