...
A scrolling list in Roblox is an `AutomaticSize` column inside an `AutomaticCanvasSize` frame, so a `ScrollingFrame` leaves its children no ceiling on an axis whose canvas is free to grow — otherwise the column comes out exactly the window's height and nothing scrolls. The bar is drawn from the same window/canvas maths the adapters feed back, so it cannot disagree with the numbers a component sized itself against.
</Callout>
