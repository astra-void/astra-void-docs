...
Everything below describes loom `0.12.0`: the JSX intrinsics in `@loom-dev/react`, the DOM mapping in
...
The engine takes the string wherever it takes the item — `AutomaticSize="XY"` *is* `Enum.AutomaticSize.XY`, and roblox-ts's own React typings offer both spellings. Loom reads either, everywhere.
</Callout>

<Callout title="`EnumItem.Value` changed in `0.12.0`">
It reports the engine's own number now, rather than the item's position in loom's declaration list.
Code that compares `.Value` against a hardcoded integer may need updating; comparing items directly,
or matching on `.Name`, is unaffected — as is the layout engine, which matches by name.
</Callout>
...
`TextWrapped` is honored: runs are laid into lines at word boundaries rather than measured as one
long line, and **one function decides every wrap** — measurement asks it how many lines a label
needs, and the text layer asks where to put the breaks it paints. The advances behind it are the
engine's, half a pixel per grapheme plus the run's kerning.
...
label, so wrapping against it never wraps at all. The library idiom stacks two or three such
containers, and the one node with a real width is where the room runs out.

The re-wrap settles inside the paint that caused it. The wrap width comes from the layout that paint
produces, so the first measurement after a container narrows is against the old width; the adapter
re-lays-out until the two agree — up to four passes, then it defers to the next frame — and patches
the DOM once. Rendering the first pass would put a too-wide label on screen for a frame, which
during a window drag is every frame.
...
engine resolves a scale inset against the node's own width where its X axis is real, and against
zero where the axis is automatic — a scale inset on an automatic axis is circular, since the width
sets the padding sets the width.
...
would draw every glyph too big by its face's own ratio. `LineHeight` sets the line box in pixels off
`TextSize`, since the engine's pitch is `TextSize`-relative. Which ratio to divide by is calibrated
per family — see [`TextSize` is not a font size](/loom/guides/fonts/#textsize-is-not-a-font-size).

The clip rect carries the overhang the browser's taller line boxes need, with padding handing the
content box its original height back — otherwise a paragraph's last line loses the tails of its `y`,
`p` and `g`. Nothing about the layout moves: `TextBounds`, `AbsoluteSize` and every rect around the
label are unchanged, and a label still clips its own text at its left and right edges.

### `AutomaticSize` is bounded by the parent

Roblox bounds automatic growth: an object with `AutomaticSize` grows up to the maximum its parent
allows, and a `TextWrapped` label grows to that maximum and only *then* wraps.

Every auto-sized node carries the ceiling its parent leaves, inherited through the padding in
between. `Size` remains the floor even past that ceiling, and a wrapping `UIListLayout` wraps
against the same ceiling. Content that still does not fit **overflows**, which is what the engine
does — it does not widen the object to make room.

**A `ScrollingFrame` is the exception.** A canvas free to grow is one whose whole point is
outgrowing the window, so its children get no ceiling on an axis carrying `AutomaticCanvasSize` or a
`CanvasSize` of 0. A `CanvasSize` giving the axis a real extent is still a ceiling.
