...
`TextWrapped` is honoured: runs are laid into lines at word boundaries, and **one function decides
every wrap** — measurement asks it how many lines a label needs, the text layer asks where the
...
is against the old width, so the adapter re-lays-out until the two agree — up to four passes, then
it defers to the next frame — and patches the DOM once.
...
axis is automatic — there the width would set the padding would set the width.
...
to descender, while CSS `font-size` sets the em square and a face's ascent plus descent runs well
past 1em.
...
The clip rect carries the overhang the browser's taller line boxes need, with padding handing the
...
background-painted element rather than an `<img>`, because a sprite window and a 9-slice both place
a *region* of the source.
...
- **In a static build** (`loom build`, `vite build`) there is no server later. The ids are resolved *at build time*, the images downloaded into the output, and a `__loom/assets.json` manifest points the page at them — see [Static builds and embedding](/loom/guides/static-builds-and-embedding/#rbxassetid-in-a-static-build).
...
A scrolling list in Roblox is an `AutomaticSize` column inside an `AutomaticCanvasSize` frame, and loom capped every child's growth at its parent's box — right for a `45%` column, wrong for a canvas. The column came out exactly the window's height, so nothing overflowed and nothing scrolled.

A `ScrollingFrame` now leaves its children no ceiling on an axis whose canvas is free to grow. The bar is drawn from the same window/canvas math the adapters feed back, so it cannot disagree with the numbers a component sized itself against.
</Callout>
...
spans the whole table on its minor axis; its own `Size` is ignored, as is any `UIPadding` on it,
which would otherwise slide its cells off the columns they define.
...
**A property read before it is written answers with a typed default**, since `0.12.0` — `0` for a
number, `""` for a string, the class's own default for a datatype — the way Roblox reflection does,
rather than `undefined`. A UI that reads `frame.BackgroundTransparency` before setting it used to get
...
`TextBox` editing all behave as in Roblox, including input-sinking.

`0.12.0` closed the gap where the rest of the mouse events were **connectable but never dispatched**,
which is worse than absent — a handler attached to one silently did nothing.
`MouseButton1Down` / `MouseButton1Up`, `MouseButton2Click` and `MouseMoved` fire now, and keyboard
input reaches the runtime at all, so `UserInputService.IsKeyDown` and a `KeyCode` comparison answer
what the user actually pressed. `Enum.KeyCode` gained its digits, modifiers and gamepad buttons in the
same release, which is what a keyboard-driven UI was missing to match against. Only `TextButton`,
...
handle, say — is hit-testable whether or not it is `Active`. A frame with no listeners stays
