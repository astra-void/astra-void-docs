...
Auto-sizing text is measured browser-side. Font metrics do not exist inside the WASM engine. The
adapter runs a canvas `measureText()` with the font the renderer paints, and hands the result over
as `TextBounds`. Which typeface that is, and why it is worth pinning, is
...
is against the old width. The adapter re-lays-out until the two agree, up to four passes before
