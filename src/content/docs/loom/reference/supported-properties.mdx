...
Everything below describes loom `0.6.4`: the JSX intrinsics in `@loom-dev/react`, the DOM mapping in
...
| `ImageRectOffset` `ImageRectSize` | Not typed | The sprite window needs more than one `<img>` layer. An icon cut out of a sheet renders as the whole sheet, and the props are absent from `ImageGuiProps`. |
...
It is a description of loom `0.6.4`, not a compatibility promise. The project is pre-1.0 and rows
