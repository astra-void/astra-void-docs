...
everywhere, as of `0.9.1`. Before that release the Scene IR encoded a string happily, but every place the adapters read one *back* off an instance insisted on an `EnumItem`. A label written `AutomaticSize="XY"` was auto-sized by the layout and measured by nobody, so it collapsed to zero and spilled its text. `FontSize` written as a string was ignored the same way.
</Callout>
...
adapter re-encodes and re-lays-out until the two agree — up to four passes, then it defers to the
next frame — and patches the DOM once, with the settled result. Rendering the first pass would put a
label wider than its container on screen for a frame — which, during a live window drag, is every
frame.
...
against zero where the axis is automatic — a scale inset on an automatic axis is circular, since the
width sets the padding sets the width. Until `0.8.1` the adapter read offsets only — the right
answer for the automatic case and wrong for the other. A wrapped label under a
...
paragraph's last line lost the tails of its `y`, `p` and `g`, and at a large `TextSize` the first
line lost the tops of its ascenders. The clip rect carries that overhang now, with padding handing
the content box its original height straight back. Nothing about the layout moves: `TextBounds`,
...
long word — pushed its container past the slot positioning it. At a narrow viewport a `45%`-wide
...
The build finds those ids two ways. It reads the emitted output for `rbxassetid://<digits>`, and — since `0.9.5` — it mounts each gallery target in node and reads the live tree, which catches an id composed at runtime (`` `rbxassetid://${iconId}` ``). An image the first render never reaches
...
A scrolling list in Roblox is an `AutomaticSize` column inside an `AutomaticCanvasSize` frame, and loom capped every child's automatic growth at its parent's box. Right for a `45%` column; wrong for a canvas, where outgrowing the window is the whole point. The column came out exactly the window's
