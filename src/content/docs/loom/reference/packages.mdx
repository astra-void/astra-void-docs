...
| `@loom-dev/renderer` | Scene IR → DOM, plus the incremental patching session with pointer and keyboard input delegation, and the font registry (`registerFont`, `onFontsChanged`). |
...
| `@loom-dev/renderer/fonts` | The openly licensed Roblox typefaces, registered on import. A side-effect module with the font packages behind it, so a project that does not import it ships none of it. See [Fonts and text metrics](/loom/guides/fonts/). |
