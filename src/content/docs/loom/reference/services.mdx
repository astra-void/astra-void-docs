...
`AnalyticsService`, `DataStoreService`, `LocalizationService`, `LogService`, `MarketplaceService`,
`MessagingService`, `PolicyService`, `Stats`, `TeleportService` and `TextChatService` are exported but
not implemented. Each is a service a browser cannot honestly stand in for — purchases, teleports,
cross-server messaging, datastores, moderation policy and telemetry all need a Roblox server on the
other end, and inventing an answer for `UserOwnsGamePassAsync` would be worse than saying nothing.

They resolve to a warned stub: one console warning at startup, then a throw naming the member on the
line that actually calls it. Leaving the name unexported instead would be a *build* error, which takes
down a preview whose only sin was importing a service beside the ones it uses. Loud where the problem
is beats fatal where it is not.
