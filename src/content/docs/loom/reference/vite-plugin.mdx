...
Absolute-path aliases are used rather than `resolve.dedupe` because dedupe would re-anchor React at *your* project root, which may hoist a different major — and the aliased `@rbxts/react` and the reconciler's React must be the same instance.
</Callout>
...
`<root>/index.html` wired up as the Rollup input, which is what makes `vite build` work in a project
...
Under `build` that is unavailable — `transformIndexHtml` runs *after* bundling, so an injected `src`
never joins the graph. Instead the HTML plugin collects the page's entry modules and prepends
`import ".../globals.ts"` to each; ESM evaluates imports depth-first in source order, so it runs
