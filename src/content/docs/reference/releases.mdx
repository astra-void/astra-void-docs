...
In-progress work on the `next` prerelease line:

- Placement-relative Popper positioning options (`placement`, `sideOffset`, `alignOffset`, `collisionPadding`) adopted across positioned overlays.
- `usePopper` now exposes resolved placement/position metadata (`position`, `anchorPoint`, `placement`, `contentSize`, `isPositioned`, `update`).
- Improved Popper flip/clamp behavior near viewport edges, and overlay content stays mounted while motion is active so exit animations finish cleanly.
- Normalized slider and switch thumb anchors so wrapped layouts keep animated thumbs aligned.

## 0.5.0

Extracted shared focus and motion helpers into dedicated packages, expanded CLI project scaffolding, and standardized motion behavior across layered primitives.
...
- Layered components share the same motion recipes and focus infrastructure, so exit animations complete instead of flashing or unmounting early.
