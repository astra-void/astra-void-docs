...
description: Every utility class Vela supports at 0.5.2, the exact values each one accepts, and the Roblox property or helper instance it lowers to.
product: vela-rbxts
section: reference
sidebar:
  order: 10
...
This is the complete list of what Vela compiles as of **0.5.2**. If a class is not on this page,
...
containing a slash (`unsupported-border-value`). A bracketed thickness — `border-[3]` — is
`unsupported-arbitrary-value` instead, because bracket payloads are parsed as arbitrary colors
first. For `ring-*`/`outline-*`: `inset`, `solid`, `dashed`, `dotted`, `double`, `offset-*`, and any
other thickness (`unsupported-stroke-value`).
...
`2xl`, `3xl`, `4xl`, `full`. There is no numeric fallback on the compile-time path: an unknown key
is `unknown-theme-key` in the `radius` family, and a bracketed one — `rounded-[8]` — is
`unsupported-arbitrary-value`. The runtime path does accept brackets; see
...
fill direction — as of 0.5.2 it is `w-*` under another name. Reach for it only when porting Tailwind
...
0.5.2 it **also** emits the ordinary `UIListLayout` carrying the same value, leaving two layout
