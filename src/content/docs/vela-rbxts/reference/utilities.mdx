...
Most utilities resolve entirely at compile time. Three families are **runtime-structural** —
margins, `divide-*` and `animate-*` build wrappers, separators or loops that cannot be static props,
so any of them moves the element onto the
[runtime path](/vela-rbxts/getting-started/how-it-works/#the-two-lowering-paths) even in a plain
...
[still open](/vela-rbxts/reference/release-notes/#still-open).

On a component none of this applies — the host it renders is unknown, so every utility is offered.
Whether the props land is up to the component, which must forward what it does not consume down to a
host element; nothing checks that it does. See
...
`DEFAULT` is not spellable in a class — `bg-slate-DEFAULT` is read as the single key
`slate-DEFAULT`.

**Arbitrary hex colors** work in every color family — `bg-[#3b82f6]`, `text-[#fff]` — taking three-
and six-digit payloads straight to `Color3.fromRGB(...)`. A bracket payload that is not a hex color
reports `unsupported-arbitrary-value`.
...
Two keywords behave specially. `transparent` sets the transparency to `1` and removes the color
property; on `placeholder-*` and `divide-*` it is `unsupported-color-key`. `current` and `inherit`
are rejected outright. Note the asymmetry that leaves on `divide-*`: `divide-white/10` resolves
while `divide-transparent` is refused — the modifier and the keyword are separate parsers. Gradient
stops report nothing at all: `from-transparent`, `via-transparent` and `to-transparent` are
...
bracketed thickness resolves on all three — `border-[3px]`, `ring-[3px]`, `outline-[2px]` — with or
without the unit. A bracket is parsed as an arbitrary *color* first, so `border-[#0f0]` is the color
and `border-[3px]` the thickness. See [arbitrary values](#arbitrary-values).
...
`2xl`, `3xl`, `4xl`, `full`. There is no numeric fallback — `rounded-8` is `unknown-theme-key` — but
a bracketed value is read directly: `rounded-[6px]` and `rounded-[6]` both give `new UDim(0, 6)`.
**A directional form writes the individual corner properties, and squares off the corners it does
not name** — so `rounded-r-lg` alone keeps the left side flat. That makes it beat the all-corner
...
Stops flush into a `ColorSequence`. `Rotation` is emitted only when non-zero, and `BackgroundColor3`
is forced to white so the gradient is not tinted by the element's own background — a `bg-*` on the
same element is overridden regardless of token order. A bad direction is
...
A parent with a `UIListLayout` — any `flex`, `gap-*` or `space-*` utility — owns child positions,
and none of these classes take effect. Remove the layout utilities from the parent instead.
...
The cross axis stays at the engine's 100px until `auto-rows-*` names it; `auto-cols-*` does the same
for `grid-rows-*`. Both read the [spacing scale](#spacing-values), so `auto-rows-24` is 96px.
...
was also Vela's own bug until [0.5.0](/vela-rbxts/reference/release-notes/#050).
...
**It fades everything the element draws:** `BackgroundTransparency` on every host,
`TextTransparency` on the text hosts, `ImageTransparency` on the image hosts, and the `Transparency`
of a `UIStroke` or `UIShadow` the same class list drew.
...
count, and so do `{props.children}` and component children, which the alpha reaches through React
context. A `canvasgroup` ends the descent — its `GroupTransparency` composites the subtree in one
pass.

**It is order-independent, and multiplies with `/N`.** The utility is held until the whole class
list is read, then composed over whatever alpha the colors settled on — so
`opacity-50 bg-blue-600/50` lands at 0.75 transparency rather than one winning. Use `/N` to fade
*one* family, `opacity-*` for the element and its subtree.
...
Two constraints. A transition needs a property that *changes* — a
[variant](/vela-rbxts/guides/responsive-and-input-variants/) rule or a dynamic `className` — so on a
...
**The property filters are real.** Each holds the tween to its own group, so a rule changing a color
and a position under `transition-colors` snaps the position and tweens the color. Transitions reach
...
Like the text families, these are enforced by the editor and the runtime path, not by the static lowering — `<frame className="scroll-y" />` emits `ScrollingDirection` onto a `Frame`, and Roblox rejects it at runtime. See [supported elements](#supported-elements).
</Callout>
...
minimum inclusive, maximum exclusive — so the two cover every viewport once, and they chain into a
...
[Configuration](/vela-rbxts/reference/config/#addvariant).

A square viewport counts as `landscape`. Input mode resolves by priority — gamepad over touch, touch
over mouse — so exactly one of `touch`, `mouse` and `gamepad` is ever active. `hover` and `active`
are tracked per element, composed with any handlers you attached yourself; `active` clears on
