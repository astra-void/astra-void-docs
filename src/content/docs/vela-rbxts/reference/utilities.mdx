...
grid track and the gap it gives back.
...
is a hard Roblox error. The static path writes it anyway — see [supported
...
resolves it and hands the children one alpha, so an `opacity-*` written inside a variant recipe
reaches the subtree it is written over.

Read [Dynamic class names](/vela-rbxts/guides/dynamic-class-names/) for the cost model and for what
promotes an element onto this path.
