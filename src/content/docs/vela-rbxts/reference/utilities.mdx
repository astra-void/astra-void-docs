...
`placeholder-*` on `textbox`, and `overscroll-*` and the [scrolling families](#scrolling) on
`scrollingframe`.
...
Whether the props land is up to the component. It must forward what it does not consume down to a
host element, and nothing checks that it does. See
...
forced to white so the gradient is not tinted by the element's own background, which overrides a
...
class written to it wins.** `ml-4 -ml-2` shifts by 8, `-ml-2 ml-4` pads by 16, and `-ml-0` clears
what came before it. `-mr-*` and `-mb-*` remain `unsupported-negative-margin` — those would have to
pull the *next* sibling closer.
...
[variant](/vela-rbxts/guides/responsive-and-input-variants/) rule or a dynamic `className`. On a
...
**The property filters are real.** Each holds the tween to its own group. A rule changing a colour
...
resolves there**, with the static path's semantics. That includes opacity modifiers, arbitrary
values, and the families that only meet at the end: the two `Size` axes, `Position` and
