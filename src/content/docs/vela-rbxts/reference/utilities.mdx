...
description: Every utility class Vela supports at 0.4.1, the exact values each one accepts, and the Roblox property or helper instance it lowers to.
product: vela-rbxts
section: reference
sidebar:
  order: 10
...
This is the complete list of what Vela compiles as of **0.4.1**. If a class is not on this page,
...
fill direction — as of 0.4.1 it is `w-*` under another name. Reach for it only when porting Tailwind
...
0.4.1 it **also** emits the ordinary `UIListLayout` carrying the same value, leaving two layout
