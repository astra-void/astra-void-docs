  ...
  rows={[
    {
      ...
      description:
        "Replaces the entire color registry. A value is either a literal Luau expression string or a map from shade (50…950) to one.",
    },
    {
      name: "theme.radius",
      type: "Record<string, string>",
      description:
        "Replaces the entire radius scale. Each value is a Luau expression producing a UDim.",
    },
    {
      name: "theme.spacing",
      type: "Record<string, string>",
      description:
        "Replaces the entire spacing scale. Each value is a Luau expression producing a UDim.",
    },
    {
...
### Values are raw Luau expression strings

Every value in the theme is a string containing a Luau expression that Vela pastes into the emitted
code. It is not a color object, not a hex string, and not a number.
...
string, and nothing checks that a string is valid Luau — it is emitted verbatim and surfaces as a
Luau error in the generated output rather than as a config error.
