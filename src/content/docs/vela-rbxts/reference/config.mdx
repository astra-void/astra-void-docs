...
<PropTable
  rows={[
    {
      name: "theme.colors",
      type: "Record<string, string | Partial<Record<Shade, string>>>",
      description:
        "Replaces the entire color registry. A value is either a literal roblox-ts expression string or a map from shade (50…950) to one.",
    },
    {
      name: "theme.radius",
    ...
    },
    {
      ...
      type: "Record<string, string | Partial<Record<Shade, string>>>",
      description:
        "Merges over the default color registry, per family and — when both sides are palettes — per shade.",
    },
    {
...
The valid shades are `50`, `100`, `200`, `300`, `400`, `500`, `600`, `700`, `800`, `900`, `950`. An
empty palette object is rejected with
`Color palette normalization requires at least one shade value.`

### Values are roblox-ts expression strings

Every value in the theme is a string containing a roblox-ts expression that Vela splices into the
TSX it emits, where roblox-ts compiles it like any other expression in your source. It is not a
color object, not a hex string, and not a number.
...
string, and nothing checks that a string is a valid roblox-ts expression — it is spliced in verbatim
and surfaces as a roblox-ts compile error rather than as a config error.
...
all eleven shades (`50` through `950`):
