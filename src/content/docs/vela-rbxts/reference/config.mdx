...
The schema is two optional keys: `preflight` and `theme`. There is no `content`, `plugins`,
`presets`, `darkMode`, `prefix`, `important`, `corePlugins`, `safelist`, or `variants` option. Those
keys do not exist in the type and are ignored if you write them anyway.
