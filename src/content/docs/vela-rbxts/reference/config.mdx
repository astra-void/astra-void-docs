...
Vela reads a single optional config file, and it configures exactly one thing: the theme. There is
no build configuration to write — file selection, output, and diagnostics all come from
...
input-shaped object it can pass through `defineConfig`. Anything else throws — and since 0.4.1 the
error names the key that failed validation, such as `theme.extend.colors.surface.55`, rather than
only reporting that a `TailwindConfig`-compatible object was expected.
