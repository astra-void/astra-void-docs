...
shift used to accumulate separately, and neither could overwrite the other. `-ml-2 -ml-2` shifted by
...
It is now one ModuleScript the whole place shares. In the reference app `App.luau` went from 190,260
...
components with `Out of local registers … exceeded limit 200`, against code nobody wrote.
...
the normal shape for a variant recipe — silently lost most of the rest. Positioning, box
constraints, the grid, gradients, rings, shadows, `z-*`, transforms, `opacity-*`, the ScrollingFrame
family and most of the text families. All of them resolve dynamically now, with the static path's
semantics.
...
carry. The static path warns `unsupported-host-utility` and writes it anyway. A build that ignores
