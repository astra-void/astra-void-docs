---
title: Diagnostics
description: Every diagnostic code Vela can emit at 0.11.0, how it reaches your build or editor, and the caveats that make some of them easy to miss.
product: vela-rbxts
section: reference
sidebar:
  order: 30
...
| `unknown-variant` | A colon prefix that is not one of the twelve variants; the message lists the supported set |
| `unsupported-arbitrary-value` | A `[...]` payload the family cannot read — `w-[abc]`, `bg-[notacolor]`, `font-[600]`. Since 0.7.0 most length families *do* read brackets; see [arbitrary values](/vela-rbxts/reference/utilities/#arbitrary-values) |
...
| `unsupported-transition-value` | An unresolvable transition/duration/delay/ease payload, including `transition-shadow` |
...
`unsupported-z-index-auto`, a negative `-z-N` or `-z-[N]` is `unsupported-negative-z-index`, a
fractional `z-[1.5]` is `unsupported-arbitrary-z-index`, and any other unbracketed number is
`unsupported-z-index-value`. A whole-number `z-[15]` resolves.

**`font-*` now falls through to `unknown-theme-key`.** Since 0.7.0 the prefix resolves the fixed
weight names first and reads anything else as a key in
[`theme.fontFamily`](/vela-rbxts/reference/config/#fontfamily), so an unrecognized payload is a
missing theme key rather than a bad weight: `font-nope`, `font-600` and `font-extrablack` all report
`unknown-theme-key` in the `fontFamily` family. `unsupported-font-weight` still exists in the code
table but no payload reaches it.

`unsupported-scroll-value` and the `transition-shadow` case of `unsupported-transition-value` both
arrived in 0.7.0 with the [scrolling families](/vela-rbxts/reference/utilities/#scrolling) and the
[transition property filters](/vela-rbxts/reference/utilities/#motion). Before that, Tailwind's
`scroll-*` reported the generic `no-roblox-equivalent` and `transition-shadow` was silently accepted
as `transition-all`.
...
`imagelabel`/`imagebutton`, `placeholder-*` only on `textbox`, `overscroll-*` and the scrolling
families only on `scrollingframe` — are checked by the editor surface alone. The transform pipeline
never consults them, so `<frame className="text-red-500" />` compiles and emits a `TextColor3`
assignment on a Frame with no complaint at all. Treat this one as a lint rule you only get in the
editor. It is also never raised on a component: the host element behind `<Panel />` is unknown, so
every utility is allowed inside its `className`.

Since 0.8.0 the **runtime path** does enforce the rule, silently: a utility the host element cannot
carry is dropped rather than written, because the assignment would be a hard Roblox error. So the
same class list can behave differently on the two paths — statically it emits an invalid property,
dynamically it emits nothing. Neither reports anything at build time.
