...
When Vela cannot lower a class, it reports it rather than dropping it quietly. This page lists every
code it can produce and what to do about the ones you will actually hit.
...
```text
[@vela-rbxts/compiler] unknown-theme-key: unknown color key "brand-500"
```
...
**Some spans are still guessed.** A diagnostic now carries a real source range whenever the compiler
could anchor it, which covers every token inside a static string `className` — those squiggles land
on the class you actually wrote, even if the same token appears elsewhere in the file. When there is
no range, the host adapter falls back to the first textual occurrence of the token anywhere in the
file. That fallback is what you get for diagnostics the compiler could not anchor: dynamic
`className` expressions, `tsx-parse-failed`, and anything the adapter synthesizes itself. In those
cases the underline can land on an unrelated comment or string, so read the message rather than the
underline.
</Callout>
...
| `color-missing-shade` | Warning | A palette family referenced without a shade |
| `color-invalid-shade` | Warning | A literal given a shade, or a palette missing that shade |
| `classname-on-unsupported-host` | Warning | `className` sits on a JSX element that is neither a supported host element nor a component |
...
on the eight supported host elements and on components; on anything else — a lowercase intrinsic it
does not implement, such as `screengui` — it leaves the attribute in the output exactly as written
and warns:
...
Move the classes onto a `frame` inside the `screengui`, or onto a component that forwards them.
...
You named a palette family without a shade. Palettes are not usable bare.

```tsx title="Fails"
<frame className="bg-slate" />
```

Write `bg-slate-800`. Only literal colors — `black`, `white`, and anything you define as a plain
string — are usable without a shade.
...
build. `bg-red` reports `color-missing-shade`, which is not filtered, but a short unknown key can be
hidden until you compile.
