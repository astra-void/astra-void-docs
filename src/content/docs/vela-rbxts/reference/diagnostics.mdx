...
— which only happens when the diagnostic carries one, in practice the dynamic-`className` case.
Everything else falls through to `{start: 0, length: 0}` at the top of the file.
...
so `font-nope`, `font-600` and `font-extrablack` all report `unknown-theme-key`.
...
raised on a component either: the host element behind `<Panel />` is unknown, so every utility is
allowed there.

The **runtime path** does enforce the rule, silently: a utility the host element cannot carry is
dropped rather than written, since the assignment would be a hard Roblox error. So the same class
list behaves differently on the two paths, and neither reports anything at build time.
...
`rounded-lg` is applied; `shadwo-lg` is dropped with a warning. The same code covers typos and
