...
**The reported span is often in the wrong place.** The span is computed as the first textual
occurrence of the offending token anywhere in the file. It is not the actual JSX attribute. If
`p-0.25` appears in three components, all three diagnostics point at the first one — and if the token
also appears in a comment or a string above the real usage, the squiggle lands there instead. Read
the message, not the underline.
</Callout>
...
rule you only get in the editor.
