---
title: Diagnostics
description: Every diagnostic code Vela can emit at 0.13.0, how it reaches your build or editor, and the caveats that make some of them easy to miss.
product: vela-rbxts
section: reference
sidebar:
  order: 30
...
[`theme.fontFamily`](/vela-rbxts/reference/config/#fontfamily). An unrecognized payload is therefore
a missing theme key rather than a bad weight: `font-nope`, `font-600` and `font-extrablack` all
report `unknown-theme-key` in the `fontFamily` family. `unsupported-font-weight` still exists in the
code table but no payload reaches it.
