...
`@vela-rbxts/runtime` is deliberately not on that list. Since 0.9.0 the inlined runtime helper lives
there as real TypeScript the repo typechecks and formats — it used to be a string literal inside the
Rust crate — and the compiler reads it at build time. The package is private and never published, so
there is still nothing of Vela's to install or to place in your DataModel.

All packages are versioned in lockstep at **0.11.0**, including the VS Code extension. There is no
