...
package: `SHADES`, `resolveThemeColors`, `mergeColorRegistry`, `normalizeColorRegistry`,
`normalizeColorScale`, and the `Shade`, `ThemeScale`, `ColorPalette`, `ColorValue`, `ThemeColors`,
`ColorInputMap`, `ThemeConfig`, and `ThemeConfigInput` types. Reach into `@vela-rbxts/config`
directly if you need them, and expect them to be less stable than the root surface.
...
Two subpaths beyond the roots are public. `@vela-rbxts/rbxtsc-host/project-config` exposes the
...
exposes the raw N-API binding underneath the compiler's default entry.
