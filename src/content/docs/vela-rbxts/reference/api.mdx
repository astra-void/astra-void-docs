...
optional; `defineConfig()` returns a fresh object deep-equal to `defaultConfig`. It is injected into
`vela.config.ts`'s scope automatically, which is why stripping the import still works.
...
That augmentation is what makes `className` type-check on the host elements, and it applies only
once the module is imported somewhere your program includes — typically a one-line `.d.ts`:

```ts title="src/vela-env.d.ts"
import "vela-rbxts";
```

Several helpers exist in `@vela-rbxts/config` but are **not** re-exported from the root: `SHADES`,
`PALETTE_DEFAULT_KEY`, `resolveThemeColors`, `mergeColorRegistry`, `normalizeColorRegistry`,
`normalizeColorScale`, and the `Shade`, `PaletteKey`, `PaletteDefaultKey`, `ThemeScale`,
`ColorPalette`, `ColorValue`, `ColorScaleInput`, `ThemeColors`, `ColorInputMap`, `ThemeConfig` and
`ThemeConfigInput` types. Reach into `@vela-rbxts/config` directly, and expect them to be less
stable.
...
directory is one of the project's `typeRoots`, and that is the one every roblox-ts project lists and
...
`^1.90.0`, and **no package declares any `peerDependencies`.** roblox-ts, TypeScript and React are
ordinary dependencies throughout, so nothing is checked at install time. You find out from a build
failure — and in the TypeScript case below, from one that does not mention versions.
...
`@vela-rbxts/rbxtsc-host` resolves `typescript` from **your** project and calls into its API directly. TypeScript 7 moved enough of that API to break the call, so a project on the current TypeScript fails on its first file with `compiler-invocation-failed: Cannot read properties of undefined (reading 'Latest')`.
...
reaching for `@vela-rbxts/compiler-wasm32-wasi`, which is not published, so it always falls through
to `Cannot find native binding.` — whose advice to reinstall without a lockfile is misleading on a
