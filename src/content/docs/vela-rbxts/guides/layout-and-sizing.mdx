...
onto those instances where it can, emulates where it can build one honestly, and warns where neither
is possible. Two rules explain most of the surprises here: a utility sets a property on the element
or on a *helper instance* Vela prepends to its children, and several Tailwind names that look like
siblings land on different Roblox properties, because Roblox splits alignment and flex distribution
into separate enums.
...
values — `justify-between`, `justify-around`, `justify-evenly` — set **`HorizontalFlex`**, a
...
the [runtime path](/vela-rbxts/guides/dynamic-class-names/)** — even in a plain string literal. The
variants:
...
`theme.spacing` ships with exactly `"4"`; every other numeric key works through the arithmetic fallback. Adding named keys is covered in [Theming](/vela-rbxts/guides/theming/).
</Callout>

<Callout title="Since 0.12.0 those offsets are rem, not pixels">
`new UDim(0, 160)` is what `w-40` is worth **at the base viewport**. Every offset on this page is a rem unit carried as `__VelaRem.scale(…)`, following the player's viewport and clamped into `[min, max]` — so these numbers hold at 1920×1020, and everywhere if you [pin the clamp](/vela-rbxts/guides/theming/#pinning-offsets-back-to-literal-pixels). Scale-valued utilities are untouched.
</Callout>
...
value with a non-zero scale component — a custom theme entry like `new UDim(0.5, 0)` — gives
...
`new UDim(1, -8)` on X — but do **not** change `AnchorPoint`, so the point they place is still the
...
`UIListLayout` owns its children's positions, so `left-*`/`top-*` will not take effect there.
...
expresses, and a **pixel** value adds to the `Position` offset. So the CSS centring idiom works
verbatim:
...
to `false`. Any other value emits `unsupported-overflow-value` — there is no scroll variant, since
scrolling belongs to the `scrollingframe` host element.
