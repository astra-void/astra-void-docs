...
The **Lowered** tab shows what Vela really emits for `font-*`: a `FontFace` carrying a Source Sans Pro `Font` value. The browser renderer behind these previews has no `Font` datatype, so the preview maps each weight onto the nearest legacy `Font` enum member instead. Relative weights read correctly; the exact face does not.
</Callout>
...
resets the style axis. Weight and style are the only two axes; the family is fixed either way.
