...
Every `font-*` class emitted the Source Sans Pro asset and nothing changed it. The workaround was to set `FontFace` directly as a prop and leave `font-*` off that element:
...
That still works, and is still the answer for a font you do not want in the theme — but it does not compose with `font-bold` or `italic`, which emit a whole `FontFace` of their own.
</Callout>
