...
description: Every variant prefix — the configurable breakpoints and their max- complements, the attribute-backed state variants, hover, active, focus and dark — how each condition is detected, and the compile-time cost of using one at all.
product: vela-rbxts
section: guides
sidebar:
  order: 50
...
`active:`, `focus:` and `dark:` arrived in 0.7.0; the `max-` forms and `attr-[…]` in 0.13.0. There is
still no `not-` form, no `disabled:`, and no group or peer variants. Order within the chain does not
matter — the conditions are combined as a set. An unrecognized prefix is its own diagnostic,
...
is not, so at exactly 768px `md:` applies and `max-md:` does not, and between them the two cover
every viewport once. They chain, which is how you address one bucket and nothing else:

```tsx title="Only between 768 and 1024"
<frame className="md:max-lg:w-1/2" />
```

A chain whose bounds leave no viewport — `lg:max-md:`, or `md:max-md:` — reports
...
treated as light. The local player is the one instance every element can reach without a provider, so
that is where the flag lives.

## State your UI owns

`hover:` is a state Roblox exposes. The states a UI has of its own — a panel that is open, a row that
is selected, a tier a player reached — have no fixed list for Vela to guess at, so **since 0.13.0 you
name them**. Both forms read a Roblox attribute off the styled instance:
...
`equals` takes a string, number or boolean. Both forms compose with every other variant, both are
checked, completed, hovered and sorted like a built-in one, and neither sends the utility behind them
to the in-game parser — `open:rounded-lg` lowers `rounded-lg` exactly as a bare `rounded-lg` does, and
only the condition travels. An `attr-[…]` that does not parse reports `malformed-attribute-variant`
...
**runtime path**. The JSX tag is replaced with a `VelaRuntimeHost` component, the variant tokens are
serialized into a `__velaRules` JSON prop, the original tag travels as `__velaTag`, and the static
props are emitted as casts.
...
ModuleScript the whole place shares. A file that needs it carries a `createVelaRuntimeHost(...)`
call and the config it hands over — a few hundred bytes, or a few thousand for a file whose host
must parse a class value and therefore keeps the full theme tables. Before that the host was inlined
...
- **Theme values are re-parsed from text.** A host that has to parse a class value reads your theme from serialized text through two narrow Luau parsers. A value that is valid roblox-ts but not shaped like `Color3.fromRGB(r, g, b)` or `new UDim(a, b)` degrades silently in game — see [Dynamic class names](/vela-rbxts/guides/dynamic-class-names/#what-the-runtime-path-still-costs).
...
interactive controls. Keep leaf components on plain static classes.
