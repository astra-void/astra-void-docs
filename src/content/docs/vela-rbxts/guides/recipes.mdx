...
**Classes** for the source, **Lowered** for what comes out — and every example stays on the
...
overwrite it with `Size = (0, 1)` scale.
...
the gaps add up to the row's inner width. `justify-between` exists (it lowers to
...
The dialog combines everything above: a column with `gap-3`, a `text-wrap` body — `TextWrapped` is
off by default, so long text clips without it — and a `justify-end` row for the buttons, which
lowers to `HorizontalAlignment = Right`.
...
corner, and `z-50` raises `ZIndex`. The scrim's transparency does not cascade — the card keeps its
