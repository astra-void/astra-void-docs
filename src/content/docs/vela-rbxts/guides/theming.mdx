---
title: Theming
description: How the Vela theme works — three families of raw Luau expression strings, the extend-versus-replace merge rules, and what ships by default.
product: vela-rbxts
section: guides
sidebar:
  order: 10
...
## Values are Luau source, not values

The single most surprising thing about the config: **every theme value is a raw Luau expression string.** It is not a color object, not a hex string, not a number. Vela pastes the string you write straight into the emitted Luau, so it has to be an expression that evaluates in Roblox.

That means colors are written as `"Color3.fromRGB(59, 130, 246)"` and radius and spacing values as `"new UDim(0, 6)"`. Writing `"#3b82f6"` or `6` does not work, but the two fail very differently. A number is rejected at config-load time — every theme value must be a `string`, so `radius: { pill: 999 }` throws while the config is being read. A string is taken at face value: nothing checks that it is valid Luau, so `"#3b82f6"` is pasted verbatim into the output and only surfaces later, as a Luau error.

```ts title="vela.config.ts"
export default defineConfig({
  theme: {
    extend: {
      // correct — a Luau expression, as a string
      colors: { ink: "Color3.fromRGB(17, 24, 39)" },
      radius: { pill: "new UDim(0.5, 0)" },
      // wrong — compiles, then breaks at runtime as invalid Luau
      // colors: { ink: "#111827" },
...
A color entry is either a **literal** — one Luau string, used as-is — or a **palette**, an object keyed by shade. The eleven valid shade tokens are `50`, `100`, `200`, `300`, `400`, `500`, `600`, `700`, `800`, `900`, and `950`. A palette does not have to define all of them, but any shade you reference must be present.
