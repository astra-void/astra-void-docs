...
Since **0.12.0** it works on every family Roblox gives a transparency channel, which is all of them but
one. `border-slate-500/25` and `ring-red-500/40` set the `UIStroke`'s `Transparency`, `divide-white/10`
fades the separator frames, and `from-blue-600/50` becomes a `UIGradient.Transparency` keypoint aligned
with its color stop — so fading one stop leaves the others alone.

Before 0.12.0 only `bg`, `text`, `image`, `shadow`, `ring` and `outline` carried it: `border-{color}/N`
was rejected by the border parser as a slash payload, and `divide-*` and the gradient stops reported
`unsupported-opacity-modifier`. A class list carried over from Tailwind lost exactly the alpha it was
written for.
...
- `border-opacity-*`. Use the `/N` modifier — `border-slate-500/25` — or `border-transparent`.
...
**It fades everything the element draws, and everything under it.** Since 0.9.0 the value reaches
every transparency channel the host paints: `BackgroundTransparency` on any element,
`TextTransparency` on the text hosts, `ImageTransparency` on the image hosts, and the `Transparency`
of a `UIStroke` or `UIShadow` the same class list drew. It is then handed down the subtree as a
running product, `1 - (1 - own) * alpha`. Since 0.10.0 that descent crosses component boundaries as
well, travelling as a React context through `{props.children}` and component children. Before 0.9.0
`opacity-*` was `BackgroundTransparency` and nothing else.
