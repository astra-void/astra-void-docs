...
property, on the host instance or on a helper Vela adds as a child. Nothing is inherited and nothing
cascades: a class writes one property on one instance.
...
`border-slate-500/25` sets the `UIStroke`'s `Transparency`, `divide-white/10` fades the separator
frames, and `from-blue-600/50` becomes a `UIGradient.Transparency` keypoint aligned with its colour
stop — so fading one stop leaves the others alone.
...
at all**: the stop is dropped and the gradient is built from what remains. Worth knowing, because a
two-stop gradient that quietly becomes one renders as a flat fill.
...
context through `{props.children}` and component children.
