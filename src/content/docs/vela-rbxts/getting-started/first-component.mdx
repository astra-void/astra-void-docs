...
The host adapter locates a diagnostic by searching for the first textual occurrence of the offending token anywhere in the file, not by the actual JSX attribute position. If the same token appears earlier in a comment or another string, the underline will point at that instead. The message and code are still correct.
</Callout>
