...
That preview is not a screenshot and not a mock-up: the example was lowered by the same compiler `rbxtsc` loads, and the emitted Roblox tree is what you see. The **Classes** tab is the source with `className` on it, the **Lowered** tab is what came out. Two differences from the snippet below, both about Roblox defaults rather than Vela: the labels carry `bg-transparent` because a Roblox text instance is opaque white by default, and the panel carries an explicit `h-24` because a width-only `Size` is zero pixels tall — which is exactly what the [sizing note](#what-each-class-lowers-to) further down describes.
