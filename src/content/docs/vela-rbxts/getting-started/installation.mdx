...
Only the `plugins` entry is Vela's. Everything else is either what roblox-ts checks or what makes JSX work. It refuses to build unless `allowSyntheticDefaultImports`, `module`, `moduleDetection`, `moduleResolution`, `noLib` and `strict` hold those exact values, `typeRoots` contains `node_modules/@rbxts`, and both `rootDir` and `outDir` are set. That list is the whole of what is validated — `target` is on roblox-ts's own enforced-options table but its check does nothing, so `"target": "ESNext"` above is a convention rather than a requirement. The three `jsx*` options are what make React JSX compile. If you already have a working roblox-ts React project, add the `plugins` entry and change nothing else.

One thing to watch as your project grows: `typeRoots` has to list **every** scope you import types from. Pull in a scoped package of your own and it needs an entry too — `"typeRoots": ["node_modules/@rbxts", "node_modules/@your-scope"]`.
