...
Two entries in there are easy to get wrong and neither is Vela-specific. `tsBuildInfoFile` requires `incremental: true` — set one without the other and TypeScript fails the build with TS5069 before Vela is ever loaded. And `typeRoots` must list **every** non-`@rbxts` scope you import types from, not just `@rbxts`; if your project pulls in a scoped package of your own, add it: `"typeRoots": ["node_modules/@rbxts", "node_modules/@your-scope"]`.
