...
import type { DocPage, SidebarSection } from "./docs";

const SITE_ROOT = "/rbxts-tailwind";
...
const SECTION_CONFIG = [
	...
	{ key: "reference", title: "Reference", slug: "reference", order: 2 },
] as const;

type TailwindDocEntry = CollectionEntry<"rbxtsTailwindDocs">;

function getDocPathFromId(id: string) {
...
function getDocUrl(path: string) {
	return path ? `${SITE_ROOT}/${path}/` : `${SITE_ROOT}/`;
}
...
function toDocPage(entry: TailwindDocEntry): DocPage {
...
export async function getRbxtsTailwindDocs() {
...
export async function getRbxtsTailwindDocsShellData(currentPath: string) {
	const docs = await getRbxtsTailwindDocs();
	const currentIndex = docs.findIndex((doc) => doc.path === currentPath);

	if (currentIndex === -1) {
		throw new Error(`Unknown rbxts-tailwind docs path: ${currentPath}`);
	}
